...
class GDPRNNBlock(nn.Module):
    def __init__(self, Cg, hidden_intra=64, hidden_inter=64, bidirectional_intra=True):
        super().__init__()
        self.intra = nn.GRU(input_size=Cg, hidden_size=hidden_intra, num_layers=1, batch_first=True, bidirectional=bidirectional_intra)
        intra_out = hidden_intra * (2 if bidirectional_intra else 1)
        self.intra_proj = nn.Linear(intra_out, Cg)
        self.inter = nn.GRU(input_size=Cg, hidden_size=hidden_inter, num_layers=1, batch_first=True, bidirectional=False)
...
class GDPRNNBottleneck(nn.Module):
    def __init__(self, C, hidden_intra=64, hidden_inter=64, repeat=1, bidirectional_intra=True):
        ...
        self.blocks = nn.ModuleList([
            GDPRNNBlock(Cg=C, hidden_intra=hidden_intra, hidden_inter=hidden_inter, bidirectional_intra=bidirectional_intra)
...
class UNetBiGRU(nn.Module):
    def __init__(self, c_in=3, base=16, groups=2, hid_intra=48, hid_inter=48, repeat_gdprnn=1):
        ...
        self.e1 = ConvBlock(c_in, c1, k=(1,5), s=(1,1))
        self.e2 = ConvBlock(c1,  c2, k=(3,3), s=(1,1))
        self.e3 = ConvBlock(c2,  c3, k=(3,3), s=(1,1))
        self.gdprnn = GDPRNNBottleneck(C=c3, hidden_intra=hid_intra, hidden_inter=hid_inter, repeat=repeat_gdprnn)
        self.d3 = DeconvBlock(c3,   c2, k=(3,3), s=(1,1), o=(0,0))
        self.d2 = DeconvBlock(c2*2, c1, k=(3,3), s=(1,1), o=(0,0))
        self.d1 = DeconvBlock(c1*2, c1, k=(1,5), s=(1,1), o=(0,0), p=(0,2))
